...
/**
 * rstbl - reads the symbol headers into data
 * @elf_header: the internal header
 * @fd: file descriptor to read
 * @i: the current symbol section to print
 */
void rstbl(elf_t *elf_header, int fd, int i)
{
    size_t r;
    char *headers;
...
/**
 * rsst - reads the string table
 * @elf_header: the internal header
 * @fd: file descriptor to read
 * @i: the current symbol section to print
 * Return: pointer to beginning of table
 */
char *rsst(elf_t *elf_header, int fd, int i)
{
    char *str;
...
/**
 * fvi - finds the verneed index
 * @verneed: the verneed table
 * @verneed_size: size of the verneed table
 * @index: index to find
 * Return: the found index
 */
size_t fvi(Elf64_Verneed *verneed, size_t verneed_size, size_t index)
{
    size_t i = 0, ret = 0;
...
/**
 * pvt - prints the verneed table
 * @elf_header: the internal header
 * @fd: file descriptor to read
 * @i: the current symbol section to print
 * @sym_string_table: the symbol string table
 * @versym: the version symbol table
 * @verneed: the verneed table
 */
void pvt(elf_t *elf_header, int fd, int i,
...
/**
 * rd - reads a block of data from descriptor
 * @elf_header: the internal header
 ...
 */
void *rd(elf_t *elf_header, int fd, unsigned long offset, long size)
{
    void *str = NULL;
