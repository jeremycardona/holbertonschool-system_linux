...
int pshf(elf_t *elf_header, int fd)
{
    ...
    if (IS_32(elf_header->e64))
        psh32(elf_header, string_table);
    else
        psh64(elf_header, string_table);
...
/**
 * rsh - reads the section headers into data
 * @elf_header: the internal header
 * @fd: file descriptor to read
 */
void rsh(elf_t *elf_header, int fd)
{
...
/**
 * rst - reads the string table
 * @elf_header: the internal header
 * @fd: file descriptor to read
 * Return: pointer to beginning of table
 */
char *rst(elf_t *elf_header, int fd)
{
    char *str;
...
/**
 * gst - return string type of section
 * @sh_type: the section type
 * Return: string name of type
 */
char *gst(unsigned int sh_type)
{
    static char buf[32];
